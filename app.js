...
let HOLIDAYS = {};          // populated by ensureHolidays()
let HOL_LOADING = false;    // true while any fetch is in flight
let HOL_PENDING_RENDER = false; // re-render queued after fetch

// Seed years already in cache into HOLIDAYS immediately (instant, no flicker)
(function seedFromCache(){
  const currentYear = new Date().getFullYear();
  for(let y = currentYear - 1; y <= currentYear + 2; y++){
    const cached = localStorage.getItem('wt4_hol_'+y);
    if(cached){
      try{ Object.assign(HOLIDAYS, JSON.parse(cached)); }catch(e){}
    }
  }
})();

// HOL_KO_DYN: Korean names from API (localName). Populated alongside HOLIDAYS.
let HOL_KO_DYN = {};
// Seed Korean names from cache too
(function seedKoFromCache(){
  const currentYear = new Date().getFullYear();
  for(let y = currentYear - 1; y <= currentYear + 2; y++){
    const cached = localStorage.getItem('wt4_hol_ko_'+y);
    if(cached){ try{ Object.assign(HOL_KO_DYN, JSON.parse(cached)); }catch(e){} }
  }
})();
...
async function fetchHolidaysForYear(year){
  ...
  if(cachedEn && cachedKo){
    try{
      Object.assign(HOLIDAYS, JSON.parse(cachedEn));
      Object.assign(HOL_KO_DYN, JSON.parse(cachedKo));
      return;
    }catch(e){}
  }
  try{
    ...
    if(!res.ok) return;
    const list = await res.json(); // [{date, localName, name, ...}, ...]
    const enObj = {}, koObj = {};
    list.forEach(h => {
      enObj[h.date] = h.name;          // English name
      koObj[h.date] = h.localName;     // Korean name from government
    });
